...
Set `braveBaseUrl`, `exaBaseUrl`, or `tavilyBaseUrl` to route those providers through a compatible API gateway. `BRAVE_BASE_URL`, `EXA_BASE_URL`, and `TAVILY_BASE_URL` are the environment-variable equivalents and take precedence over config. HTTPS is recommended; explicitly configured HTTP URLs work with any valid host, including `http://client-gateway`, `http://10.0.0.1:8080/api`, and loopback addresses. **HTTP sends API keys in plaintext over the network**. These explicit endpoints can reach internal addresses, so configure only trusted gateways. Brave appends `/web/search`, Exa appends `/answer` or `/search`, and Tavily appends `/search`. Defaults remain the providers' official HTTPS API roots. `exaBaseUrl` applies only to keyed direct API calls; zero-config Exa MCP search continues to use Exa's hosted MCP endpoint. Invalid overrides fail before a request is sent instead of falling back to an official endpoint, and credential headers are removed if a gateway redirects to another origin.
